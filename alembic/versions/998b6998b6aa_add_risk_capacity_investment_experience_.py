...
def upgrade() -> None:
    op.execute(
        "ALTER TABLE investment_profiles ADD COLUMN IF NOT EXISTS liquidity_needs TEXT"
    )
    op.execute(
        "ALTER TABLE investment_profiles ADD COLUMN IF NOT EXISTS income_needs NUMERIC(15, 2)"
    )
    op.execute(
        "ALTER TABLE risk_profiles ADD COLUMN IF NOT EXISTS risk_capacity VARCHAR(50)"
    )
    op.execute(
