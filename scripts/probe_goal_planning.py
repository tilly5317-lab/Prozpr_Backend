"""DEV-ONLY: Probe the entire goal_planning pipeline end-to-end.
...
What it does:
  ...
  3. If ANTHROPIC_API_KEY is available, runs the agent (run_goal_planning_agent) with
     4 sample user queries in ONE session (so the checkpointer threads state).
  4. Dumps every intermediate to /tmp/goal_planning_probe/ as JSON.

Run from worktree root:
    /Users/Amoul/.../.venv-mac/bin/python scripts/probe_goal_planning.py

JSON outputs land in /tmp/goal_planning_probe/.
"""
...
# Wire up dotenv from the parent project's .env (the goal_planning module's worktree
# inherits .env via the parent backend repo).
try:
    from dotenv import load_dotenv
    # From scripts/probe_goal_planning.py:
    #   parents[0] = scripts/
...
OUT_DIR = Path("/tmp/goal_planning_probe")
    ...
    return GoalPlanningInput(
        ...
        current_properties=[
            CurrentProperty(
                ...
                mortgage_last_date=date(2040, 5, 9),  # 14 years remaining
            ),
        ],
        goal_properties=[
            GoalProperty(
