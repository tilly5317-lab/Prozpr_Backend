...
| `named_fund` | "use fund X" / "why not fund X?" | Why-not → answer from `get_rejection_reasons()` / `selection_reason`; unranked → honest no-rank. **Inclusion ships in Phase 2 at the input-builder seam** (inject the fund as a ranked row so the engine computes real numbers — reshape-level substitution would fabricate row fields); Phase 1 answers include-asks honestly ("coming later" + why we picked ours) and measures demand via telemetry |
...
- **After the engine** — `allowed_categories`, `excluded_categories`, `category_weight_targets`, `target_fund_count`: generalize `compute_reshaped_buys` from drop-and-redistribute to "meet each requested category's minimum share, then distribute the remainder pro-rata to survivors". **The legacy-only path (allowed/count, no new fields) runs the original F3-B code byte-for-byte** — new capability is layered around the core, never through it. Invariants — **scoped to this after-engine reshape only**: total buy preserved exactly, sells never touched, rounding residual to the largest buy, identity when nothing binds.
- **Composition order** for stacked preferences ("only equity, more mid cap, 4 funds"): (1) tilts/scope pre-engine → (2) eligibility filters (excluded / allowed) → (3) category weight targets → (4) fund count, where the count-trim must not evict a category named by a weight target (an unsatisfiable count bumps up to the protected-category count, disclosed). Cross-key contradictions ("only debt, and more mid cap") → clarify mode, naming the specific conflict.
- **Named-fund inclusion seam:** injecting a customer-named fund into the plan happens at the **input builder** (the fund enters as a ranked row; the engine computes real buy/tax numbers for it) — never at the reshape, which would have to fabricate response-row fields. Phase 2 work; until then include-asks get an honest deferred reply and `named_include_deferred` telemetry.
