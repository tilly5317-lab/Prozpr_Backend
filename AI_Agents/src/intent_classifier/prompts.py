...
5. **A DIRECTIVE to adjust the risk of an ACTIVE plan → keep that plan's intent.** While rebalancing or a new-money deployment is the active intent, an imperative to change exposure — "make it safer", "more conservative", "more aggressive", "push my equity up", "crank up the equity", "dial down the risk" — adjusts the plan just produced (the anaphoric "it"/"my" is that plan) and keeps the active intent. This holds EVEN when the customer justifies it with their age or risk appetite ("I'm 28 and want to be aggressive, push my equity up" is a directive to tilt the active plan, not a request to redesign their profile). Only a QUESTION about what the target SHOULD be — "should I be more aggressive given my age?", "what mix is right for me?" — is `asset_allocation`.
