...
def _apply_goal_mutation(
    inp: GoalPlanningInput, goal_name: str, fields: dict
) -> GoalPlanningInput:
...
def generate_lever_a_increase_sip(
    ...
    baseline_out: GoalPlanningOutput,
    ...
    for _ in range(8):
        mid = (lo + hi) / 2
        new_inp = inp.model_copy(deep=True)
        new_inp.profile = inp.profile.model_copy(
            update={"starting_monthly_investment": mid}
        )
    ...
    confidence = (
        "high"
        if target_sip < 2 * base_sip
        else ("medium" if target_sip < 3 * base_sip else "low")
    )
    return Lever(
...
def generate_lever_c_reduce_target(
    ...
    for pct in range(5, int(reduce_max_pct * 100) + 1, 5):
        ...
        new_inp = _apply_goal_mutation(
            inp, largest.name, {"goal_value_pv": new_goal_value_pv}
        )
        new_out = compute_full_projection(new_inp)
        if _is_feasible(new_out):
...
def generate_lever_e_step_up(
    inp: GoalPlanningInput,
    baseline_out: GoalPlanningOutput,
    step_up_max_delta_pp: float = 0.20,
) -> Lever | None:
    ...
    for _ in range(8):
        mid = (lo + hi) / 2
        new_inp = inp.model_copy(deep=True)
        new_inp.assumptions = inp.assumptions.model_copy(
            update={"annual_invested_amount_growth": mid}
        )
        new_out = compute_full_projection(new_inp)
        if _is_feasible(new_out):
            best = (mid, new_out)
...
def generate_lever_f_reduce_expense(
    ...
    for pct in reduce_pct_list:
        ...
        new_inp = inp.model_copy(deep=True)
        new_inp.profile = inp.profile.model_copy(
            update={"monthly_household_expense": new_expense}
        )
        new_out = compute_full_projection(new_inp)
        if _is_feasible(new_out):
            return Lever(
