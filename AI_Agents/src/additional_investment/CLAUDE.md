...
- **Bucket targeting (legacy path).** `select_target_bucket` picks the nearest unfunded horizon short → medium → long (long is the fallback); `compute_targets` weights subgroups by that bucket's column. Which horizons count as "unfunded" is decided by the caller, not this engine — the app builder can force long-term. Emergency is never a target; a targeted bucket with no allocation ⇒ fully undeployed (`ratio.py`).
