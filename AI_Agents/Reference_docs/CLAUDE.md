...
- `fund_house_commentry.md` — Prozpr's monthly fund-house **view** (qualitative stance on equities / debt / gold), **hand-maintained**, never agent-generated or overwritten. Read as a file by `src/portfolio_query/` and by `app/domains/market_commentary/services/fund_house_view_module_service.py` (the market chat flow). Whether view vs the factual `.md` loads is gated by the classifier's `tools_needed` (`fund_house_view` vs `market_commentary`). Chat answers surface the named houses as **research sources** (Prozpr leads and advises), never as recommendations.
