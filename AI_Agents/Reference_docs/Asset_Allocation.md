...
*Version 1.2 · Internal & client reference · Last updated: May 2026*

---

> **Status note for follow-up questions:** This document is the client-facing thesis for the goal-based allocation engine that lives in `ailax/Prozpr_Backend/AI_Agents/src/asset_allocation_pydantic/`. As of v1.1 the **emergency fund**, **short-term goals** and **negative-NFA carve-out** sections have been rewritten to match the live engine. **As of v1.2 the ELSS first-pass has been removed from the allocation logic** — the engine no longer allocates to ELSS regardless of tax regime or 80C headroom. Other paragraphs (intergenerational override, multi-asset 65/35 framing, ₹16L hybrid gate, sector cap %s, "1% drop" rule, medium-term valuation overlay) still describe the *intent* of the thesis and may run ahead of what the code does today — for engine-true behaviour cross-check `tables.py` and the `steps/` files.
...
| **6. Tax-efficient by default** | Tax outcomes are baked into the allocation, not bolted on. Hybrid funds replace pure debt for higher-effective-tax-rate clients to defer tax drag, and arbitrage-plus-income funds lift post-tax returns on the medium- and long-term debt sleeve. We consider time horizon and the tax impact of selling securities at that horizon when picking funds. |
...
| **Why hybrid (multi-asset) funds in particular?** | Equity-tilted hybrid funds get used because: (i) tax efficiency on the debt portion (capital-gains taxation rather than slab income tax); (ii) diversification across large-cap, mid-cap, small-cap, sometimes international equity and commodities; (iii) lower churn and transaction cost — the manager rebalances within the fund. |
