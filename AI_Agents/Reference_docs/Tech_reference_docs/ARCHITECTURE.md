...
- **In:** `RebalancingComputeRequest` — corpus, tax state (STCG budget, carryforward losses), an optional `asset_class_tilt` map (a customer preference — see *Investment preferences* below), and a single homogeneous list of `FundRowInput` rows. Recommended funds carry `rank ≥ 1`; held-but-not-recommended ("BAD") funds arrive in two flavours, both `is_recommended = False`: **force-exit** — `rank = FORCE_EXIT_RANK` (9999), `target_amount_pre_cap = 0`; step 2 sets `exit_flag`, step 4 liquidates regardless of tax — and **NEUTRAL** — `rank = 0`, `target_amount_pre_cap = st_value_inr` (the locked ST minimum), so `diff = -lt_value` and only the migratable LT portion ever reads as sellable. Two things live on the request as **scalars**, not rows, and have special handling:
...
- **Investment preferences — before the engine (asset-class tilt).** Phase 1, spec [`2026-08-24-investment-preferences-design.md`](../../../docs/superpowers/specs/2026-08-24-investment-preferences-design.md). When a customer states a preference in rebalancing chat ("increase my equity", "only equity funds"), `RebalancingComputeRequest.asset_class_tilt` carries an optional `{class: percent}` map (`equity`/`debt`/`others`, keyed through `SUBGROUP_TO_ASSET_CLASS`). `pipeline._apply_asset_class_tilt` scales the practical allocation's `aggregated_subgroups` to that mix *before* subgroup targets are lifted onto rank-1 rows — within-class proportions are preserved, and a requested class with zero current total re-spreads its share over the classes present (grand total stays exact). It reshapes **only the targets view** (`practical_for_targets`); the response's `practical_allocation` passthrough stays engine-*recommended* — the comply-and-caution baseline. A tilt legitimately changes what is sold, so a tilt turn runs the whole engine **twice** in the app bridge ([`rebal_engine/chat.py`](../../../app/domains/rebalancing/services/rebal_engine/chat.py) `_handle_preference_counterfactual` — recommended baseline + requested plan, both `persist=False`); the deviation between the two is the caution lens (`constraint_impact.build_constraint_impact`).
- **Investment preferences — after the engine (category weights / exclusions).** `consolidation.py`'s F3-B buy-side reshape (redistributes only the *buy* budget across the funds the customer allows; every sell and the total buy are preserved; identity when nothing binds) gains `excluded_categories` + `category_weight_targets` on `ConsolidationConstraints`. `compute_reshaped_buys` dispatches: allowed-list / fund-count asks keep the original F3-B arithmetic **byte-for-byte** via `_reshape_legacy`; the new fields take `_reshape_extended` (eligibility filters → weight targets → count trim, in the same frozen-base + rounded-delta + residual-onto-largest-buy form). New capability is layered *around* the legacy core, never routed *through* it.
- **The LLM stays a pure classifier; magnitudes are policy, not judgement.** The app-layer follow-up detector (`chat.py` `_detect_rebal_action`, Haiku) gains the preference vocabulary (`tilt_asset_class`, `tilt_delta_pp` / `tilt_target_pct`, `scope_only_asset_classes`, `excluded_categories`, `category_weights`, `named_fund`) but only ever extracts a number *or its absence* — it **never emits `asset_class_tilt` itself**. `mutual_funds/services/investment_preferences.normalize_tilt` folds scope + tilt into one absolute mix, and a policy table fills any missing magnitude: a number-less tilt lands at the customer's risk-band edge (never beyond it; absent a risk profile it falls back to the AA default score), `_DEFAULT_WEIGHT_STEP = 10%` for a number-less "more X". The applied default is flagged in `applied_preferences` so the formatter discloses it, and the shared answer formatter still writes the reply. Stateless throughout (`persist=False`); a detected-but-unservable ask (redirect, unranked category, deferred named-fund include) fires the **content-free** `preference_unserved` PostHog event (`capture_preference_unserved` — `flow` + a fixed `failure_class` token + `session_id`, never chat text; reviewers join to the Postgres transcript by session id). Named-fund *inclusion* is Phase 2 (input-builder seam); Phase 1 answers include-asks honestly and measures the demand.
...
16. **A rebalancing tilt reshapes the targets view, not the passthrough.** Under `asset_class_tilt`, `pipeline._apply_asset_class_tilt` bends only `practical_for_targets`; `RebalancingComputeResponse.practical_allocation` still ships the engine-*recommended* mix. So a preference turn's facts pack is dual-source — the ideal/practical figures are the recommended baseline, the trade rows are the *requested* plan (its sells and tax differ) — and the formatter is told to present the two side by side, never blended or averaged. Don't "fix" the passthrough to match the tilt: that divergence *is* the comply-and-caution comparison.
