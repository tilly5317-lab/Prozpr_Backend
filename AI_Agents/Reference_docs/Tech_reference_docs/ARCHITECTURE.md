...
flowchart TB
    subgraph Shared["📦 Shared foundation — src/"]
        ...
        TS["token_stream.py<br/>incremental answer deltas"]
    end
    ...
    subgraph Query["💬 Q&A surfaces"]
        PQ["portfolio_query<br/>Haiku + skill markdown + guardrails"]
        MFQ["mutual_fund_query<br/>extract → narrate, two forced-tool passes"]
    end
    ...
    RefDocs -.->|"read only when tools_needed<br/>asks for it"| PQ
    IC -.->|"tools_needed<br/>(wired by caller)"| PQ
...
> **Edges out of the *Shared foundation* box are omitted on purpose.** Any agent may import `common.py`, `persona.py`, `reasoned_reply.py` and `token_stream.py`, so drawing them would cross the whole diagram. Today `common.py` is imported by market_commentary, Rebalancing, asset_allocation_pydantic, portfolio_query, mutual_fund_query, risk_profiling and cashflow_statement; `persona.py` by market_commentary, portfolio_query, risk_profiling, asset_allocation_pydantic's rationale step and the cashflow summarizer; `reasoned_reply.py` by market_commentary's document generator and `chat_qa`; `token_stream.py` by portfolio_query's and mutual_fund_query's LLM clients (and, through the app-layer re-export, by the answer formatter and general_chat). The "agents are peers except where a solid arrow exists" rule is about *agent-to-agent* edges, not these.
...
| **LLM-driven** | `intent_classifier`, `market_commentary` (extraction, doc-gen, and the `chat_qa` Q&A), `portfolio_query`, `mutual_fund_query`, `cashflow_statement/agent` (LangGraph — **not on the production path**; exercised only by its own tests and `scripts/probe_cashflow_statement.py`. The production goal-planning turn calls the engine directly via `compute_full_projection` and does its LLM work in the app layer: a Haiku action detector in `goal_planning_engine/chat.py` plus the shared answer-formatter), `risk_profiling` (summary half), the optional rationale step in `asset_allocation_pydantic` | Claude via `langchain-anthropic` is the load-bearing piece — Haiku for most calls, Sonnet for the cashflow agent node and market-commentary Q&A. Outputs are constrained by structured-output schemas or prompt-encoded house rules, but they're not byte-stable. Debug them by reading prompts in `prompts.py` and looking at recent runs. |

**Every LLM call site pins `temperature=0`.** Omitting it applies the Anthropic API default of **1.0**, and the sampling was doing visible damage: "when do I reach ₹10 crore?" answered FY2034/₹10.44cr and FY2035/₹11.85cr on consecutive calls with identical data, and 4 of 101 labelled turns changed intent between identical classifier runs. `app/domains/ai_engine/tests/test_temperature_is_pinned.py` scans every `ChatAnthropic(...)` call under `app/` and `AI_Agents/src/` and fails on any that omits it. The one indirection is the answer formatter, which passes its kwargs as a dict and reads `AILAX_FORMATTER_TEMPERATURE` — the env var is an escape hatch, but its **default is `"0"`**, and a second test pins that default. If you add an LLM call site, pass `temperature` explicitly as a literal or the scan will fail.
...
These pieces aren't agents — they're support: one numeric library and four shared modules.
...
### `persona.py`, `reasoned_reply.py`, `token_stream.py` — shared LLM plumbing

Three more src-level shared modules (added after `common.py`; same rule — keep them small):
...
- [`token_stream.py`](../../src/token_stream.py) — the answer-token stream for one chat turn, re-exported to the app layer by `app/domains/ai_engine/streaming.py`. See §4.1.

### 4.1 `token_stream.py` — how an answer streams

Every customer-facing answer surface produces its text through a **forced tool call**, so the reply arrives as partial JSON inside a tool argument, not as plain text. This module turns that into incremental deltas.

It lives under `src/` rather than `app/` for the same reason `persona.py` does: one of the producing surfaces is `portfolio_query/llm_client.py`, an agent, and **agents must not import the app layer**. Canonical definition here, re-exported there.

| Piece | What it does |
|---|---|
| `TokenStream` | Async-iterable sink of answer-text deltas for one turn. The queue is **unbounded on purpose** — bounding it would drop deltas from the middle of an answer and silently corrupt the rendered text; the producer is capped by the call site's `max_tokens` anyway. |
| `open_token_stream()` | Async context manager that installs a stream for the current context and always closes it on the way out, so a consumer can never hang on a turn that already died. |
| `current_token_stream()` | Reads the active stream, or `None`. |
| `astream_tool_answer(llm, messages, answer_field=…)` | Runs `llm.astream` over a forced-tool call, parsing the half-built tool-call JSON each chunk and publishing the growth in `answer_field`. Returns the accumulated message, which duck-types what `ainvoke` returns — callers keep reading `.tool_calls` and `.response_metadata` unchanged. |
| `FINE_GRAINED_TOOL_STREAMING` | The Anthropic beta flag that makes any of this worth doing (see below). |

Four things that are load-bearing:

1. **Nothing streams unless a stream is open.** With no open stream every call site keeps using `ainvoke` on exactly the path it used before, so the blocking endpoint is byte-for-byte unaffected. The beta flag is likewise set *only* while streaming.
2. **The beta is required, not an optimisation.** By default the API buffers a tool call's input JSON server-side and flushes it near the end of the response, which makes token streaming nearly pointless — measured on a ~4,500-char answer, the first 10% of text arrived 11.55s into a 15.86s call. With the beta the same answer delivered 10% by 2.71s of 11.94s. The trade-off it buys: input JSON is no longer validated before it is sent, so a malformed tool call is possible — both call sites already degrade safely to their deterministic fallback, so the cost is a lost formatter run, not a broken reply.
3. **Identity travels in a `ContextVar`, not an argument.** The answer-producing call sites sit five-plus frames below the request handler and neither takes a `TurnContext`. `asyncio.create_task` copies the context, so a turn spawned inside `open_token_stream` inherits the sink.
4. **Deltas are PROVISIONAL — whatever `run_turn` returns wins.** The formatter discards a truncated response in favour of a deterministic brief, and general_chat renders its `answer` field into a different final shape. A client that has painted deltas must replace them with the final content.

**Single-instance only**, the same trade-off as `app.core.progress`: the sink lives in process memory, so the streaming request and the turn must share a worker. The Dockerfile runs one uvicorn worker — do not add `--workers` without moving the sink out of process.

The producing call sites today: `answer_formatter/formatter.py` (`_invoke_llm`), `general_chat`'s compose pass, `portfolio_query/llm_client.py` and `mutual_fund_query/llm_client.py` (both via an explicit `stream_field`, so an internal field is never streamed to a customer by accident). One consequence worth knowing: the **streaming path reports usage on `usage_metadata`** and leaves `response_metadata["usage"]` empty, so both agent clients carry a fallback — without it, token and cost accounting silently records zeros for every streamed turn.
...
- **Out:** `ClassificationResult` (one of 9 intents, confidence, reasoning, optional `out_of_scope_subreason`, and `tools_needed`).
- **How it thinks:** [classifier.py](../../src/intent_classifier/classifier.py) builds an LCEL chain over Claude Haiku with structured output. The system prompt in [prompts.py](../../src/intent_classifier/prompts.py) enumerates the intents and gives examples; reasoning is capped at ~12 words so it can't eat the token budget. The chain returns a parsed `ClassificationResult`. That's it — one model call.
- **LLM:** Claude Haiku, structured output, `temperature=0`, Anthropic prompt caching on the system prompt. `max_tokens` stays at 1024 as pure headroom — hitting the cap truncates the tool call and raises rather than degrading.
- **The nine intents** are the enum in [models.py](../../src/intent_classifier/models.py): `asset_allocation`, `goal_planning`, `stock_advice`, `portfolio_query`, `general_market_query`, `rebalancing`, `additional_investment`, `mutual_fund_query`, `out_of_scope`. **The enum is the source of truth.** If a doc says any other count, trust the enum.
- **Out-of-scope has sub-reasons.** `OutOfScopeSubreason` (same `models.py`): `gibberish`, `identity_or_meta`, `security_or_credentials`, `chat_summary`, `off_topic`, `other`. The app layer picks a different canned/tailored reply per sub-reason. **`out_of_scope` is not a fallback** — the prompt requires a positive reason (a domain we don't handle, non-financial chatter, adversarial input). Merely short, vague or ambiguous is explicitly *not* sufficient; those resolve from `active_intent` or history instead.
- **`tools_needed` — a second, independent verdict.** `Tool` (same `models.py`) is a small enum: today `market_commentary` (current factual market DATA) and `fund_house_view` (Prozpr's stance plus the major fund houses' outlooks, surfaced to the customer as research sources). It answers "what extra data does the ANSWER stage need?", and **it does not affect the intent**. The brain copies it onto `TurnContext.tools_needed` and handlers fetch only what is listed; empty means the customer's own record is enough. Two implementation notes that bite: the guidance for this field lives in the pydantic `Field(description=…)` and **not** in `prompts.py`, because the same text in the system prompt primes the market intent and misroutes benchmark questions; and a new `Tool` member also needs `_ToolLiteral` in `classifier.py`, which a drift test enforces. Consumed by `flow_market` (the market-query flow) and `portfolio_query` (§5.8).
...
- **What it does NOT decide: `action_mode`.** A natural but wrong assumption. `ClassificationResult` has no mode field. Routing and mode are separate LLM decisions in separate places — the mode is chosen *inside* each module after routing, by that module's own detector.
...
- **A second, hand-maintained file — the fund-house view.** [Reference_docs/fund_house_commentry.md](../fund_house_commentry.md) holds Prozpr's monthly qualitative stance plus the outlooks of major fund houses (ICICI, HDFC, PPFAS, …). Unlike the factual `.md` it is **not** machine-generated — a human updates it monthly. `flow_market` and `portfolio_query` load it (whole) when the classifier's `tools_needed` asks for `fund_house_view`, and the answer **surfaces the named houses as research sources** to build confidence (Prozpr still leads/advises). Read via `app/domains/market_commentary/services/fund_house_view_module_service.py` — a plain file read that degrades gracefully when absent.
...
- **How it thinks:** six steps in [steps/](../../src/asset_allocation_pydantic/steps/) — the files keep their historical numbers, so there is no step 3 (the medium-term bucket was removed 2026-09-24) — orchestrated by [pipeline.py](../../src/asset_allocation_pydantic/pipeline.py):
  ...
  | 2 | [step2_short_term.py](../../src/asset_allocation_pydantic/steps/step2_short_term.py) | Allocate to goals below the FY-anchored short/long boundary (instruments lean debt/arbitrage). |
...
- **The short/long horizon boundary is FY-anchored.** A goal is short-term below `HORIZON_BOUNDARY_MONTHS + months_to_fy_end` and long-term at or above it ([step2_short_term.py](../../src/asset_allocation_pydantic/steps/step2_short_term.py), [step4_long_term.py](../../src/asset_allocation_pydantic/steps/step4_long_term.py)) — the flat 24-month line is pushed out to the current financial-year end. `AllocationInput.months_to_fy_end` defaults to 0, so the engine tests and no-preference goldens see the flat line; the production [`aa_engine/input_builder.py`](../../../app/domains/asset_allocation/services/aa_engine/input_builder.py) threads the real offset from today, making the boundary live for customers (spec 2026-09-25).
...
This is the first cross-agent import in the package, the only one that mutates the allocation math, and **the only engine that honours a customer's standing investment preferences**.

- **In:** `PracticalAllocationInput` — `AllocationInput` plus four scalars (`mf_corpus`, `non_mf_equity_corpus`, `elss_corpus`, `max_non_mf_equity_pct_client_input`) and an optional `human_override` carrying the customer's resolved preference.
- **Out:** `PracticalAllocationOutput` — shape-parity with `GoalAllocationOutput` plus a `corpus_breakdown` block and an optional `human_override_applied` report. Consumers that handle `GoalAllocationOutput` handle this with zero change for the shared seven fields.
- **How it thinks:** [pipeline.py](../../src/practical_asset_allocation/pipeline.py) (one file on purpose — the models, the orchestrator, and the R157–R222 long-term math together, past 1,000 lines; the math is easier to follow contiguously, so don't split it for tidiness). It calls `asset_allocation_pydantic` steps 1–3 and 5 directly, lifts selected step-4 helpers (`phase1_bounds`, `phase2_asset_class_pcts`, `phase4_multi_asset`, `phase5_equity_subgroups`), and **reimplements step 4 itself** to be holdings-aware:
...
- **Investment preferences — honoured as INPUTS, at the phase that decides each facet** (spec 2026-09-14). This replaced a post-hoc reshape known as "step 6", which ran *after* the whole allocation was built. By then the multi-asset sleeve was carved and frozen, so the reshape could not move debt: an 85/7/8 ask landed at 78.33/14.00/7.67 on a real portfolio, and where a buffer did exist it could only hit the target by gutting it (₹3,00,000 → ₹1,653 on one fixture). The facets now enter separately:

  | Facet | Where it lands |
  |---|---|
  | Asset-class split | `phase2_asset_class_pcts` — the requested split **IS** the output; the market-view tilt and the phase-1 bounds (the others-gate included) are bypassed |
  | Multi-asset sleeve size | `phase4_multi_asset` |
  | Sub-group pins / exclusions | `phase5_equity_subgroups` |

  Gold and the long-term debt bucket are pure residuals of their class, so pinning either has the same shape as an equity pin — a ceiling on the sleeve. Every sub-group preference therefore collapses into one rule: *the sleeve is the smallest size any class room allows*, and each residual is what it leaves behind. Sub-group shares are a % of the **whole portfolio**, never of their class ([D-A3](../../src/practical_asset_allocation/human_override.py)); a share of `0` is a **hard exclusion**, not a pin of zero. Excluding gold zeroes the commodity class and with it the sleeve — the fund holds gold, so "no gold" means no fund. Excluding a debt bucket re-routes to the other one rather than stranding money in a refused fund.
- **Setting any preference suspends the steps 1–3 carve-outs.** Emergency fund, near-term goals and the NFA liability offset stop being carved, and the whole corpus goes to the long-term step (`_no_carveout_buckets`, spec 2026-09-15 §3). This is a real trade the customer is making, so `carve_outs_at_risk()` reads the profile for both the screen's pre-commit warning and the record attached to the run — one source, so the two cannot disagree.
- **`apply_human_override` is now the reporting point, not a reshape.** It is always invoked and is a strict no-op on empty preferences (the golden-test guarantee: a no-preference run must stay byte-identical to before this change). What it reports is what only it knows — whether anything stopped the ask landing: over-subscribed pins scale proportionally, while an over-large multi-asset sleeve is *capped* instead, because the sleeve is one fund rather than a share of a class pool.
- **The ideal engine stays preference-free.** `asset_allocation_pydantic` remains Prozpr's own recommendation; preferences shape only the practical, holdings-aware plan. That is what makes "here's what we'd advise, here's what you asked for" a truthful contrast rather than two views of the same tilted number.
- **LLM:** none.
...
- **In:** `RebalancingComputeRequest` — corpus, tax state (STCG budget, carryforward losses), and a single homogeneous list of `FundRowInput` rows. (The old `asset_class_tilt` map is gone — see *Investment preferences* below.) Recommended funds carry `rank ≥ 1`; held-but-not-recommended ("BAD") funds arrive in two flavours, both `is_recommended = False`: **force-exit** — `rank = FORCE_EXIT_RANK` (9999), `target_amount_pre_cap = 0`; step 2 sets `exit_flag`, step 4 liquidates regardless of tax — and **NEUTRAL** — `rank = 0`, `target_amount_pre_cap = st_value_inr` (the locked ST minimum), so `diff = -lt_value` and only the migratable LT portion ever reads as sellable. Two things live on the request as **scalars**, not rows, and have special handling:
...
- **How it thinks:** [pipeline.py](../../src/Rebalancing/pipeline.py) calls `practical_asset_allocation.run_practical_allocation` first (this is why the import edge exists), then splits per-subgroup targets across the top 1-2 ranked fund rows (`_funds_per_subgroup` / `_residual_shares`) — an overwrite, not a fill: whatever the input builder set there is discarded, and the target is reduced (floored at zero) by the ST value of that subgroup's held-but-not-recommended `rank = 0` rows, because that locked ST is already exposure to the subgroup and skipping the offset double-allocates; the frozen subgroups (`tax_efficient_equities`, `non_mf_equities`) are exempt from the lift entirely — then runs the six steps in [steps/](../../src/Rebalancing/steps/):

  | Step | File | What it does |
  |---|---|---|
  | 1 | [step1_cap_and_spill.py](../../src/Rebalancing/steps/step1_cap_and_spill.py) | Rounds the subgroup targets the pipeline already split across the top 1-2 funds. No longer caps or spills — the per-fund cap does not bound a rebalance (spec 2026-09-20); the module keeps its historical name. |
...
- **Investment preferences — no longer applied here (2026-09-14/17).** The per-turn `asset_class_tilt` map, `_apply_asset_class_tilt`, the `practical_for_targets` reshape and the twin-run counterfactual are all **deleted**. A standing customer preference now reaches this engine only through the upstream practical allocation it rebalances toward (§5.5) — one carrier, applied once, at the phase that decides each facet. A drift test fails on any reappearance of the old vocabulary: [`test_preference_extraction.py::test_legacy_sleeve_tilt_is_gone_everywhere`](../../../app/domains/rebalancing/tests/test_preference_extraction.py) greps `app/` and `AI_Agents/src/` for `asset_class_tilt`, `market_cap_tilt`, `pure_equity_only`, `scope_only_asset_classes` and friends.
- **Chat no longer runs preference what-ifs.** The Haiku follow-up detector (`chat.py` `_detect_rebal_action`) still *extracts* `preference_asks` — the schema is kept deliberately as the re-enable seam, so don't delete it as dead code — but a filled `preference_asks` now produces the one shared `PREFERENCE_REDIRECT_MESSAGE` plus `show_preferences_pill`, and fires the **content-free** `preference_unserved` PostHog event (`capture_preference_unserved` — `flow` + a fixed `failure_class` token + `session_id`, never chat text). A *servable* override in the same turn (cash injection, tax rate) is still run and answered: returning the pointer alone dropped a real question. Saved preferences still shape the plan and are still disclosed in the reply.
- **Consolidation survives, narrowed.** `consolidation.py`'s F3-B buy-side reshape still redistributes only the *buy* budget across allowed funds (every sell and the total buy preserved; identity when nothing binds), and `ConsolidationConstraints` still carries `excluded_categories` + `category_weight_targets`. In production today only `target_fund_count` reaches it, from the fund-count consolidation ask — the category fields have no live caller since the preference path was retired.
...
| Node | What it does |
|---|---|
| `ingest_baseline` | Resets per-turn audit fields (`actions_taken_this_turn`, `extracted_events_this_turn`). Compares the new baseline's **profile block** against the cached `last_output` (`last_output.input_echo.profile != baseline_input.profile`); on a mismatch it discards the cache (`last_output` set to `None`) and flags state as `dirty` (dropping an orphaned override also sets `dirty`). Note the comparison is profile-only — a changed goal, property, mortgage or one-off event does not by itself invalidate the cached projection, which `compute_projection` short-circuits on whenever state is not `dirty`. It also carries an orphan check that applies only to overrides exposing a `property_name`, validated against `current_properties` + `goal_properties` — no current `OverrideSpec` variant (`NumericOverride`, `RateOverride`) has that attribute, so the branch never fires and nothing is dropped today. Goal-name orphans (e.g. a `GoalMutation` naming a deleted custom goal) are never validated here. |
...
That whole sequence is three LLM calls (two agent turns plus `summarize_plan` in `finalize`, which runs every turn) + one engine run. The graph's job is to wire them up so the LLM can decide *during the turn* what to do, instead of you having to hand-write the if/else.
...
- **In:** `ClientContext`, `PortfolioContext` (asset-class + sub-category + per-fund detail), the question, optional conversation history, and `want_market_commentary`.
...
- **The market commentary is now GATED — this is the classifier's `tools_needed` in action.** Loading it unconditionally put fund-house valuation views into every "review my portfolio", and the model reached for them: it compared a 26.11% small-cap *allocation* against a "35.5x expensive *valuation*" — a percentage against a P/E. The chat bridge passes `want_market_commentary="market_commentary" in ctx.tools_needed`; when false the commentary block is replaced by an explicit placeholder telling the model to answer from holdings and profile, not to speculate about markets, and **not to mention that commentary is missing**. `want_market_commentary` defaults to `True`, so non-chat callers are unchanged.
- **Two different XIRRs reach the prompt.** `holdings[].xirr_pct` is one fund's annualised return; the portfolio-level `xirr_pct` is the whole portfolio's. Per-fund values are looked up from `UserMfLatestSnapshot` keyed by `scheme_code` (which is `ticker_symbol` on the holding) and are populated for mutual funds with enough transaction history — so the model *can* rank and compare funds by it. The lookup returns `{}` on any failure by design: a missing XIRR degrades one line of an answer and must never fail the turn.
- **LLM:** Haiku via [llm_client.py](../../src/portfolio_query/llm_client.py) (a thin `ChatAnthropic` wrapper), `temperature=0`, prompt caching, forced tool-use to constrain the output to `PortfolioQueryResponse`. It streams when the turn has an open token stream, and the caller must name the field: `stream_field="answer"`. Only `answer` is customer-facing — `redirect_message` is substituted wholesale by the guardrail backstop, so streaming it would show text the validator may then null.
...
- **How it thinks:** [pipeline.py](../../src/additional_investment/pipeline.py) switches between two split modes, then selects funds:
  ...
  - **Bucket targeting** (SIP, or lumpsum without holdings): `select_target_bucket` picks the nearest unfunded horizon (short → long — the medium branch survives in the enum but never fires now the bucket is gone), and the deploy follows that bucket's subgroup weights.
  - [selection.py](../../src/additional_investment/selection.py) then splits each subgroup's target across its **top 1-2 ranked funds** by corpus (2 at/above ₹50L of investable, else 1 — the same threshold as rebalancing; spec 2026-09-24), nearest-₹100 rounding. The per-fund cap and the SIP-mirrors-rebalancing path are retired. BUY-only; it never sells.
...
### 5.10 `mutual_fund_query/` — questions about funds themselves

Triggered by the `mutual_fund_query` intent (added July 2026). "What are Parag Parikh's returns?", "how does it compare to peers?", "which are the best large cap funds?" land here.

The distinction that governs routing: **`mutual_fund_query` is about funds in the abstract, held or not; `portfolio_query` is about funds the customer owns.** The prompt's scope test is to read the possessive first — a superlative does not override it, so "the best funds" is a screen but "my best funds" is `portfolio_query`. And a *verdict* on their existing holdings ("do I have the right funds?") is `rebalancing`, not either of these.

- **In:** the question, optional conversation history, and — for the second pass only — a `MutualFundQueryFacts` container built by the app layer.
- **Out:** `MutualFundQueryResponse` — `answer` plus an optional `clarifying_question` the bridge falls back to when `answer` is empty.
- **How it thinks:** [orchestrator.py](../../src/mutual_fund_query/orchestrator.py) runs **two single forced-tool passes — no agentic loop**:
  1. `extract` — name the fund(s) with pronouns resolved from context, and classify the ask as `reasoning` / `returns` / `comparison`. It also sets `is_screen` (plus optional `screen_category` / `screen_horizon_years`) for a generic "best performing funds" question that names no fund. **This pass must not answer the question**, only identify it.
  2. `narrate` — render the grounded facts into the reply under the guardrail rules, wrapped in the shared PI voice via `persona.build_system_prompt`.
- **The engine is DB-agnostic — it only ever sees DTOs.** Between the two passes the app layer resolves fund names and assembles `MutualFundQueryFacts` (per-fund returns, our `house_reason` and `shortlist_rank` where the fund is on our shortlist, and peer rows). Nothing under `src/` touches the database.
- **`shortlist_rank` is our house rank, NOT a category percentile.** The field name invites the wrong reading; `has_house_view` is what tells you whether we have an opinion at all.
- **The screen path leaves the agent entirely.** When `extract` sets `is_screen`, the app service calls `screen_top_funds` and answers through the shared answer formatter with `action_mode="screen"` — `narrate` never runs.
- **All prompt content lives in one file.** [mutual_fund_query.md](../../src/mutual_fund_query/mutual_fund_query.md), split into four sections (Extract System / Extract User / Narrate System / Narrate User), with the guardrail rules embedded in Narrate System. Read with `read_text_bom_aware` (§4).
- **LLM:** Haiku for both passes via [llm_client.py](../../src/mutual_fund_query/llm_client.py), `temperature=0`, forced tool-use. `narrate` streams with `stream_field="answer"`; `extract` does not stream — it produces no customer-facing prose.
...
flowchart LR
    ...
    MC_out["market_commentary output<br/>{MacroSnapshot + markdown file}"]
    ...
    PREF["saved_investment_preferences row<br/>(app-side, resolved)"]

    RP_out -->|"caller fills fields"| AA_in
    PREF -->|"caller resolves → human_override"| AA_in
    MC_out -.->|"score block NOT wired — engine defaults used"| AA_in
    MC_out -.->|"writes file (read only when tools_needed says so)"| PQ_in
    AA_in -->|"runs"| PA_out
    PA_out -->|"per-subgroup targets split across top 1-2 rows"| RB_in
    PA_out -->|"post-investment ideal subgroups<br/>+ holdings snapshot + ranking"| AINV_in
```

The five things to internalise:

1. **`AllocationInput` is the join point — but only the risk half is actually wired.** It carries fields from both `risk_profiling` and `market_commentary`, and neither of those agents imports allocation; the calling domain service does the joining. Today that joining is one-sided: `app/domains/asset_allocation/services/aa_engine/input_builder.py` fills the risk fields (`effective_risk_score`, `osi`, `savings_rate_adjustment`, `gap_exceeds_3`, read off the persisted `effective_risk_assessments` row), but **no code in `app/` ever sets `market_commentary`** — the symbol `MarketCommentaryScores` does not appear anywhere under `app/`. The field falls through to its `default_factory`, so every production allocation runs on the neutral house view in `asset_allocation_pydantic/tables.py` (`DEFAULT_MARKET_COMMENTARY_SCORES`, all 5.0). The engine consumes those scores for real (`steps/step4_long_term.py` phase-2 asset-class percentages and the subgroup/others gates) — it just always sees neutral. Wiring a live view is a caller-side capability nobody has connected yet. Note also that the agent's own output model is `MacroSnapshot` (macro indicators + `document_md`) — it emits **no** 1–10 score block at all, so a `MarketCommentaryScores` would have to be *derived* from the snapshot by a caller that does not exist.
2. **`market_commentary` writes a file; `portfolio_query` reads it — when it is asked to.** No code-level coupling. If you change the file, you change a contract. Since the `tools_needed` gate (§5.1, §5.8), the read is conditional: a portfolio question that needs no market view now gets a placeholder instead of the commentary.
3. **`Rebalancing` builds on `practical_asset_allocation`'s output.** It calls `run_practical_allocation` first, then splits subgroup targets across the top 1-2 ranked rows.
...
5. **The customer's preference is caller-wired too, and it has exactly one load point.** `PracticalAllocationInput.human_override` is filled by `app/domains/practical_asset_allocation/services/paa_engine/input_builder.py` off the preloaded `User.saved_investment_preference` relationship — engines never touch the DB. A contract test allows only that one computation-time reader; a second would make historical runs unreproducible. Note the interaction with point 1: when a class preference is present, `phase2_asset_class_pcts` takes the requested split verbatim, so the neutral market scores are bypassed rather than merely neutral.
...
- **Shared src-level modules are exactly four:** `common.py` (INR formatting, risk categories, BOM-aware reads), `persona.py` (the PI voice — `build_system_prompt`), `reasoned_reply.py` (thinking-first forced-tool-use replies), `token_stream.py` (answer-token deltas — §4.1). Any agent may import these; keep them small, don't grow dumping grounds, and don't add a fifth without a design discussion. The bar each one cleared: an agent under `src/` needed it, so it could not live in `app/`.
- **No LLM call may sample.** Pass `temperature=0` explicitly at every `ChatAnthropic(...)` site — see §3. A test scans for this and fails the build.
- **The customer never sees an internal section name.** The prompt sections we assemble — `CUSTOMER_RECORD`, `PROFILE`, `LOGIC_REFERENCE`, `ACTION_MODE`, `MODULE` — and the field names inside them are how *we* organise inputs; they mean nothing to a customer. The house rule now stated in every answer prompt: when you can't answer, name the missing thing in the customer's own terms ("I can't see the returns on each individual fund yet"), never the plumbing ("that isn't in my CUSTOMER_RECORD", "the information provided to me"). The section was called `FACTS_PACK` until August 2026; the Python identifier `facts_pack` still is, so expect the two names to coexist — the rename was to the *prompt* text only.
...
1. **The intent count keeps growing — trust the enum.** Currently **nine** intents (`additional_investment` and `mutual_fund_query` both joined in July 2026), plus the `OutOfScopeSubreason` sub-enum and the separate `Tool` enum behind `tools_needed`. Older docs/comments say six, seven or eight. The enum in [models.py](../../src/intent_classifier/models.py) is the only source of truth.
...
12. **Streamed deltas are not the answer.** `run_turn`'s return value is authoritative — the formatter throws away a truncated response in favour of a deterministic brief, general_chat re-renders its `answer` field into a different final shape. Any client that paints deltas must replace them on `done`. Don't "optimise" that away.

13. **The token-stream sink is in process memory, so chat is single-worker.** Same constraint as `app.core.progress`. The Dockerfile runs one uvicorn worker; adding `--workers` silently breaks streaming (and progress) for every request that doesn't happen to land on the right process. Moving the sink out of process is the prerequisite, not a follow-up.

14. **`temperature` omitted means 1.0, not 0.** The most expensive one-line mistake in this codebase — it produced different rupee figures for the same customer on consecutive calls. Landmine, not preference: see §3.

15. **`ActionMode` no longer has `recompute` / `recompute_full`.** A re-run is `compute` with `is_rerun: true` in the facts pack. The mode list also gained `consolidate`, `screen`, `category_probe` and `gather` — three of which had been firing from modules *outside* the declared union before `action_mode` was type-enforced. Enforcing the type is what surfaced a goal-planning bug where an empty clarification question fell through and computed a full projection.

16. **The goal-ideal ships on the first answer only.** `rebal_engine/service.py` gates it behind `include_ideal`, which `chat.py` sets to `action_mode == "compute"`. Follow-up turns therefore contrast against the *recommended practical plan*, never the goal-ideal — a follow-up that re-quoted the ideal read as the plan having changed when nothing had. A preference-shaped plan is disclosed separately, by `preference_view.active_preferences_block`, whose `applied` / `shortfall_reason` come from the **run** (`human_override_applied`) rather than the saved row, so the claim always describes the plan actually in front of the customer; an output with no such field — AA's preference-free ideal fallback — yields `None` rather than a claim.
...
| Change what a fund-level question answers with | [mutual_fund_query/mutual_fund_query.md](../../src/mutual_fund_query/mutual_fund_query.md) (all four prompt sections) + the app-layer facts builder in `app/domains/mutual_funds/services/mutual_fund_query_service.py` |
| Make an answer surface stream | [token_stream.py](../../src/token_stream.py) §4.1 — call `astream_tool_answer` with an explicit `stream_field`, and set the beta only while streaming |
| Add a new `tools_needed` signal | [intent_classifier/models.py](../../src/intent_classifier/models.py) (`Tool` enum) + `_ToolLiteral` in `classifier.py`, then consume it off `TurnContext.tools_needed`. Put the guidance in the pydantic field description, **not** `prompts.py` |
