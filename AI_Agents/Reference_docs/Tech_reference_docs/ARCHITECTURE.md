...
Consumers today: `cashflow_statement/engine/` plus the app layer's benchmark/TWR service (`app/domains/portfolio/services/benchmark_service.py`). **Don't engineer it for hypothetical consumers** — extend only when a real caller appears (`AI_Agents/src/CLAUDE.md` makes this an explicit rule).
...
> **✅ Wired: [`willingness.py`](../../src/risk_profiling/willingness.py) is the production willingness source.** This module exports `compute_risk_willingness` — the scorer for the **4-question customer risk-willingness questionnaire** (Q1 investment preference, Q2 investment experience, Q3 investment focus, Q4 drop reaction → 1–10 float via `min(mean(Q1,Q3,Q4), lift, Q2_cap)`). The app layer's [`derive_risk_willingness`](../../../app/domains/profile/services/_effective_risk/inputs.py) calls it with the customer's `risk_level` (Q1) and the three behavioural answer sentences (Q2–Q4).
>
> Why it matters: `scoring.py` still consumes `risk_willingness` as an *already-computed* float on `RiskProfileInput`. `derive_risk_willingness` resolves that float — preferring an explicit stored `RiskProfile.risk_willingness`, then the Q1–Q4 model, then the legacy [`risk_willingness_from_risk_level`](../../../app/domains/profile/services/_effective_risk/calculation.py) only when the model yields nothing. So editing the bands, the lift rule, the experience cap, or the frontend→key reference tables in `willingness.py` moves live `effective_risk_score`.
>
> **Do not** reinvent the Q1–Q4 mapping in the app layer or the frontend — the bands, the lift rule, and the experience cap are product decisions encoded here. The three text reference tables must stay byte-exact with the frontend `BEHAV_Q*_OPTIONS` (a `test_willingness.py` guard enforces this); a reworded or legacy option is scored as unanswered, not an error.
...
- **LLM:** Haiku for extract + generate. There is also a third surface: [chat_qa.py](../../src/market_commentary/chat_qa.py) — a **Sonnet** Q&A sub-agent that answers customer questions over the commentary (forced tool-use via `reasoned_reply`), used by the market-query chat flow.
...
6. Done. Returns a `GoalPlanningSnapshot` to the app layer. The chat layer's responder LLM (a separate call, outside this graph) writes the actual customer reply using the snapshot as input.
...
- **`models.py` is the boundary.** Top-level pydantic schemas live here. The app layer imports only from `models.py` and from documented entry points (`pipeline.py`, `main.py`, `orchestrator.py`, `classifier.py`). It never reaches into `steps/` or `engine/`.
...
1. **The intent count keeps growing — trust the enum.** Currently **nine** intents (`additional_investment` then `mutual_fund_query` joined in July 2026), plus the `OutOfScopeSubreason` sub-enum. Older docs/comments say six or seven. The enum in [models.py](../../src/intent_classifier/models.py) is the only source of truth.
