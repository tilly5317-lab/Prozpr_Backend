...
flowchart TD
    ...
    C -->|"one of the 7 real question types"| D["Route: pick the matching flow recipe"]
...
- **The answer can now arrive word-by-word.** There are two ways to ask the same question. The original one does the whole turn and returns the finished reply in one piece. A newer one streams: the customer sees progress lines while the specialists work, then watches the answer being typed out as it's written. Both run the *identical* turn — the streaming version is a different delivery pipe, not a different brain.
  - One caveat worth knowing, because it shapes what the app does: **the streamed words are provisional.** In a few cases the finished reply differs from what was streamed — if the AI's answer gets cut off mid-way the system throws it away and substitutes a pre-written factual summary, and general chat re-shapes its answer before returning it. So the app paints the streaming text as it arrives, then replaces it with the final version when the turn completes.
...
**The commentary is no longer loaded for every portfolio question.** It used to be pulled in whenever a customer asked about their own holdings, and that caused real mistakes — in one case the AI compared a customer's 26% small-cap *holding* against a "35.5x expensive *valuation*", which are not comparable numbers. Now the receptionist decides: it flags whether answering actually needs a market view, and only then is the commentary fetched. A question answerable from the customer's own portfolio gets no market view at all — and the AI is explicitly told not to speculate about markets, and not to mention that it's missing.

**Two kinds of market context — and the customer now hears real fund-house views.** Alongside the daily factual snapshot there is a second, hand-maintained source: Prozpr's monthly **fund-house view** — our qualitative stance on equities, debt and gold, together with the published outlooks of major fund houses (ICICI, HDFC, PPFAS and others). The receptionist decides which a question needs — the numbers, our view, or both. When a customer asks what we think, the answer now **names those fund houses as research sources** — *"our view is cautious on small-caps, and ICICI and Canara flag the same, though PPFAS is more constructive"* — which builds confidence by showing that reputable houses back the view. Prozpr remains the one advising; the houses are cited as outlooks, never as recommendations.
...
- **Redo** — re-run the plan from scratch with their current saved details. (Internally this is no longer a separate mode: a re-run is an ordinary compute with a "they've seen this before" flag, so the reply opens by acknowledging the re-run and leads with what changed.)
...
| **Customer Record** | The bundle of computed numbers a specialist hands to the Answer Writer — the only figures it's allowed to cite. Called the *Facts Pack* until August 2026, and still named that in the code. |
