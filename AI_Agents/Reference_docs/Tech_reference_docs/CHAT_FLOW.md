...
flowchart TD
    ...
    C -->|"one of the 6 real question types"| D["Route: pick the matching flow recipe"]
...
- **The answer arrives all at once**, not word-by-word streaming. The customer waits while the work happens, then sees the complete reply.
...
|---|---|---|
| **Asset allocation** | "How should I invest? What mix is right for me?" | Builds an ideal investment mix and explains it. *Saved.* |
...
| **Portfolio query** | "What's in my portfolio? What's my biggest holding?" | Answers factually from their actual holdings. *Read-only.* |
...
The last two types are special: **no specialist plans anything for them**. But they're not always word-for-word canned any more. For stock-advice and general off-topic questions, the shared Answer Writer re-words the standard redirect so it briefly acknowledges what the customer actually asked before steering back. Truly unanswerable inputs — gibberish, security probes, identity questions, "summarise this chat" — keep a fixed, pre-written reply: fast and consistent.
...
Most question types use **one** specialist. Two of them use **two specialists in a chain**, where the first one's result is handed to the second:
...
flowchart LR
    ...
    IC -->|general market query| M1["Market Commentary"] --> M2["General Chat writes the reply"]
...
- **Market questions** need *fresh facts first*. So the **Market Commentary** module gathers the latest market data, then hands it to **General Chat**, which writes the actual answer grounded in those facts.
- **Rebalancing** needs a *target* before it can plan trades. So **Practical Allocation** first works out the ideal mix *given what the customer already owns*, then **Rebalancing** figures out which funds to buy and sell to get there.

When a flow chains two specialists, the first specialist's output is passed forward to the second — they never reach into each other directly. This keeps each specialist independent and easy to reason about.
...
It also applies real-world constraints: tax-saving (ELSS) funds that are locked in for three years are handled carefully, and limits are placed on certain holdings. It runs entirely on fixed rules (no AI guesswork in the maths). The customer sees the same kind of mix-and-table reply — and crucially, this becomes the **target** that the Rebalancing module aims for. *(This module runs as the first step of the rebalancing flow; customers don't trigger it by name.)*

### Rebalancing
Turns a target mix into an **actual trade plan**. Given the ideal mix (from Practical Allocation) and the customer's current holdings, it produces the specific list: *buy ₹X of this fund, sell ₹Y of that one*.
...
- it's a category we **never deploy fresh chat money into** (e.g. ELSS, because of its 3-year lock-in — the picks are still named, the policy is stated), or
- we simply **don't rank funds in that category** — said plainly, nothing invented.

Every category answer carries the same standing caveat: concentrating in one category isn't what we'd recommend — the plan spreads money across the customer's goals. If they name a category but no amount, the reply gives the picks and asks how much they'd like to invest.
...
In a chat turn, this commentary is produced first and handed to General Chat to write the final reply. Two edits in AI_Agents/Reference_docs/Tech_reference_docs/CHAT_FLOW.md.

Line 158 — replace:
"In a chat turn, this commentary is produced first and handed to General Chat to write the final reply. If gathering fresh data takes too long (more than ~2 minutes), the system **doesn't make the customer wait** — it falls back to the most recent saved snapshot and continues, so the customer still gets an answer."

with:
"In a chat turn, this commentary is produced first and handed to General Chat to write the final reply. If a saved snapshot from the last day is still current, it is used as-is — no fresh gathering, no wait. Otherwise fresh data is gathered, and if that takes too long (more than ~2 minutes) the system **doesn't make the customer wait**: General Chat is handed no commentary and answers from its own quick web search instead, so the customer still gets an answer."

Line 260 (Example C) — replace:
"**Path:** understand → *general market query* → Market Commentary gathers fresh macro data (or falls back to the latest saved snapshot if it's slow) → General Chat writes the reply using that data → answer. ✅"

with:
"**Path:** understand → *general market query* → Market Commentary reuses the last day's saved snapshot if it's still current, otherwise gathers fresh macro data (and if that's too slow, hands over nothing) → General Chat writes the reply using that data, falling back to its own web search if no commentary arrived → answer. ✅"

After editing the .md, regenerate the viewer with `python3 -m scripts.build_reference_docs`; do not hand-edit CHAT_FLOW.html.
...
- **Redo** — re-run the plan from scratch with their current saved details.
...
- **Redirect** — when they want something chat can't change from here (like editing a goal), point them to the right place instead.

So a follow-up turn really happens in **two steps**: first a quick sorting into one of these *modes*, then the actual writing. Replace the last sentence of CHAT_FLOW.md:182 with:

"For allocation and rebalancing, the very first question about a topic skips the sorting — the specialist simply builds the plan — so there the modes only come into play on follow-ups. Goal planning sorts on every turn, the first one included, because a customer's opening question can already be a what-if ('what if I retire at 50?') or too vague to run ('what if I retire earlier?' — earlier than what?)."

Optionally also soften the glossary entry at CHAT_FLOW.md:336, which repeats the follow-up-only framing: change "On a follow-up question, the *kind* of answer needed" to "The *kind* of answer needed — explain, teach, what-if, redo, ask-back, or redirect."
...
flowchart TD
    subgraph IN["What the Answer Writer is handed"]
      ...
      P2["The Facts Pack — the exact numbers the specialist computed (it may ONLY use these)"]
      P3["The customer's profile — first name, situation"]
      P4["The last 6 messages — recent context"]
    ...
    W -->|"if it fails or is too slow"| FB["A pre-written, fact-based fallback summary"]
    FB --> R
```

Notice that the Writer's instructions come in **two halves**: a *shared house style* that every specialist uses (so the voice never changes), plus a *module-specific briefing* that only makes sense for that one specialist — an allocation briefing explains what an "equity/debt mix" means, a rebalancing briefing explains what a "buy/sell trade" means. On top of that, when the customer is in *explain* or *teach* mode — asking how or why the approach works — the four planning specialists (allocation, rebalancing, goal planning, additional investment) also hand over a **third piece**: Prozpr's published methodology write-up for that topic, philosophy only, with none of the proprietary thresholds, caps or weights in it. The Writer is told to ground those "how does this work" explanations in that write-up and nothing else, so the reasoning a customer reads is what Prozpr actually publishes rather than the model's general knowledge. The customer never sees any of the three — only the finished reply. — an allocation briefing explains what an "equity/debt mix" means, a rebalancing briefing explains what a "buy/sell trade" means. The shared half keeps Prozpr sounding like one person across every kind of question; the specific half lets each answer be accurate. The customer never sees either — only the finished reply.

**The rules that keep answers trustworthy:**

- **No making things up.** The Writer is told it may only cite numbers that are in the Facts Pack. If it's asked *how* a figure was derived and the underlying rate isn't in the pack, it describes the result without inventing the method.
...
- **A safety net.** If the Writer fails for any reason (an error, or it takes too long), the system falls back to a **pre-written, deterministic summary** built straight from the facts — so the customer always gets a coherent answer, never an error page.
...
- Different parts use different amounts of that history: the **receptionist** (classifier) sees the last ~12 messages, and the **Answer Writer** sees the last ~6 — enough to stay relevant without being overwhelmed.
...
sequenceDiagram
    ...
    B->>AA: build the ideal mix
    AA-->>B: target mix + chart data (saved)
    B->>AF: write the reply from these facts
    AF-->>B: friendly explanation
    B-->>C: recommended mix + pie chart
```

**Path:** understand → *asset allocation* → Asset Allocation module → Answer Writer → saved reply with a chart. ✅

### Example B — "What's my biggest holding?"

**Path:** understand → *portfolio query* → Portfolio Query module reads the customer's holdings → factual answer. **Nothing is saved** (read-only). If the question had secretly been "which stock should I buy more of?", the module's guardrail would politely redirect instead. ✅
...
**Path:** understand → *rebalancing* → Practical Allocation works out the ideal mix given current holdings → Rebalancing builds the tax-aware buy/sell plan → saved reply with the recommended trades. ✅
...
**Path:** understand → *additional investment* → the module reads the amount and cadence (₹5 lakh, one-time) from the question → compares current holdings against the ideal mix → recommends specific funds that top up the below-target areas → Answer Writer explains the picks → saved reply. ✅
...
sequenceDiagram
    ...
    AF-->>C: names the picks, asks "how much, lumpsum or SIP?"
...
**Path (turn 2):** the module reuses the small-cap ask from the previous turn, builds the real plan for ₹5 lakh, and tells the truth about the category — even when the honest answer is "your plan adds nothing here." ✅ *(This exact conversation is live-validated — it was previously the flow's worst failure case.)*
...
> ⚠️ **This section is for the internal team, not customers.** Producing this flow chart doubled as a sanity check of the live code. The architecture is healthy overall (see the verdict below). The original review surfaced one real issue and two minor notes; the "save it" issue and its related stale-comment note have since been **resolved** by removing the feature (see below), leaving one minor product note open.
...
### Resolved since the original review — the "save it" two-turn path
...
| **Facts Pack** | The bundle of computed numbers a specialist hands to the Answer Writer — the only figures it's allowed to cite. |
| **Mode** | On a follow-up question, the *kind* of answer needed — explain, teach, what-if, redo, ask-back, or redirect. The AI picks it before writing, and it shapes how the reply reads. |
