...
| **5. Mortgage purchases are upfront + EMI stream, never a balloon** | When a goal property is bought on a mortgage, the corpus pays only the *upfront* on the goal date; the rest becomes a monthly EMI flowing through the projection until the loan is paid off (using standard Indian EMI mechanics). The same property goal can be a sharp one-time corpus drain (cash purchase) or a long, even drag on monthly savings (mortgage) — and we show you both shapes correctly rather than averaging them. |
...
| **7. The horizon stretches as far as your latest goal — no further** | The core question is "can you retire on plan, *and* will every goal still get its money?" Goals dated after retirement are not dropped — the projection runs out to your latest goal and the corpus left after the retirement payout continues to fund them. Post-retirement *living costs* are shown for context but are represented by the retirement corpus itself, so we don't double-count them. One-off events scheduled beyond the plan's end are dropped, with a clear warning. |
...
For each property you *want* to buy: a cash purchase takes the full price out of the corpus on the goal date; a mortgage purchase takes only the upfront amount then, with the remainder becoming an EMI stream over the loan's life.

### Stage 5 — Goals table

We combine retirement, property goals and custom goals into a single table with shared fields — today's value, future value, the corpus required, the horizon-appropriate return, and the present-value of what's needed — so every goal is compared on the same footing.

### Stage 6 — Cashflow projection

We walk every month from today to the end of the horizon, producing a row per month: income (growing year over year, and stopping at retirement), taxes, living costs (stepped up with inflation), mortgage EMIs, savings before and after EMIs, and any one-off flows.
...
| **Will my goal scheduled after retirement still be funded?** | Yes. The projection runs out to your latest goal, and the corpus remaining after the retirement payout keeps compounding and funds it on its date. Post-retirement living costs are shown for context but aren't deducted again — the retirement corpus already represents them. One-off events past the plan's end are dropped, with a warning. |
