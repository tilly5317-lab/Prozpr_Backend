...
SECTION_GROUPS = [
    ...
    (
        ...
        [
            ...
            "3Y rolling 2025",
            "3Y rolling 2026",
            "%ile rank 3Y rolling 2020",
            "%ile rank 3Y rolling 2021",
            "%ile rank 3Y rolling 2022",
            "%ile rank 3Y rolling 2023",
            "%ile rank 3Y rolling 2024",
            "%ile rank 3Y rolling 2025",
    ...
    (
        "Benchmark returns",
        [
            "Bench 3Y rolling 2020",
            "Bench 3Y rolling 2021",
            "Bench 3Y rolling 2022",
            "Bench 3Y rolling 2023",
            "Bench 3Y rolling 2024",
            "Bench 3Y rolling 2025",
    ...
    (
        ...
        [
            "size_exposure_LMS",
            "style_exposure_GV",
            "top10_holdings_weight_pct",
            "alpha_selection_l3y_pct",
            "alpha_allocation_l3y_pct",
            "portfolio_churn_l3y_pct",
        ],
    ),
    (
        "4. Costs & Efficiency",
...
def process_one(row: dict, log) -> dict:
    ...
    try:
        ...
        t3 = extract_tier3(
            detail, (out.get("isin_growth", ""), out.get("isin_div_reinvest", ""))
        )
        out.update(t3)
    except Exception as e:
...
def build_xlsx(
    funds: list[dict], output_path: Path, source_csv: Path | None = None
) -> None:
    ...
    cols = [
        c for c in cols if c not in {"schemeCode", "schemeName"} or cols.index(c) <= 1
    ]
    ...
    for c in cols:
        ...
        total = len(df)
        cov.append(
            [c, int(filled), int(total), round(100 * filled / total, 1) if total else 0]
        )
...
def main():
    ...
    p.add_argument(
        "--codes", nargs="+", type=int, help="Subset of scheme_codes (pilot)"
    )
    p.add_argument("--out", default="build/output/mf_evaluation.xlsx")
    p.add_argument(
        "--checkpoint",
        default="build/output/_partial.csv",
    ...
    with open(log_path, "w") as log:
        for i, (_, row) in enumerate(df.iterrows()):
            ...
            if i % 25 == 0 or elapsed > 3:
                print(
                    f"  [{i + 1}/{len(df)}] {code} {row['schemeName'][:55]:55s}  {elapsed:.1f}s"
                )
