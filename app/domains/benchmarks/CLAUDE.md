...
Owns all benchmark *data*. Portfolio analytics that *consume* it (portfolio-vs-Nifty) live in `portfolio/` and read from here via `benchmark_data_service`.
...
- **EOD lag is intentional.** Reads use "nearest `value_date <= on`" (`get_value_on_or_before`), so a lookup for *today* returns yesterday's EOD until tonight's close publishes. The portfolio comparison relies on this.
