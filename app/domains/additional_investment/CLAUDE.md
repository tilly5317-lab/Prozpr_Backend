...
- **Lumpsum runs deficit-fill against real holdings** (`ainv_engine/service.py`, `AINV_ENGINE_VERSION = "ainv-2.0.0"`). One `HoldingsSnapshot` supplies both the `CorpusPin` that pins PAA at holdings + deploy (total/MF/non-MF-equity/ELSS corpus) and the engine's `current_value_by_subgroup`; per-subgroup ideal/current/gap/buy facts surface as `outcome.deficit_facts` for the formatter. SIP keeps the legacy profile-corpus path. No snapshot fallback by product decision (CAMS upload mandatory) — a snapshot failure propagates.
