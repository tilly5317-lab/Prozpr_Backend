...
- **"When do I reach ₹X?" is resolved in code, not by the prompt.** The detector extracts `milestone_target_inr` (guidance in the Pydantic field description, never the prompt body) and `_corpus_milestone` writes the first crossing into `facts_pack["corpus_milestone"]`. Asking the formatter to scan 20-40 `corpus_closing` rows for the first crossing is what answered "when do I reach ₹10 crore?" with FY2034 and FY2035 on consecutive calls — do not put the scan back in `_BODY_PROMPT` (`service.py`, `chat.py`).
