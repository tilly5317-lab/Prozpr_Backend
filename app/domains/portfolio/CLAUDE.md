...
- **The market commentary is no longer fetched for every portfolio turn.** `answer_portfolio_query` passes `want_market_commentary` **and** `want_fund_house_view` from the classifier's `ctx.tools_needed` — judgement questions ("am I too aggressive?") load the fund-house view (surfaced in the reply as research sources); unconditional commentary loading once made the model compare an allocation % against a P/E. Commentary defaults `True`, view defaults `False`, so a caller that forgets restores the old behaviour (`services/portfolio_query_service.py`).
