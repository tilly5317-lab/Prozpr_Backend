...
from app.domains.mutual_funds.services import aa_import_service
...
async def list_aa_imports(
    ...
    return await aa_import_service.list_imports(
...
@router.post(
    "/", response_model=MfAaImportResponse, status_code=status.HTTP_201_CREATED
)
...
async def update_aa_import(
    ...
    return await aa_import_service.update_import(
        db, import_id, current_user.id, payload
    )
...
async def list_aa_summaries(
    ...
    return await aa_import_service.list_summaries(
...
async def get_aa_summary(
    ...
    return await aa_import_service.get_summary(
        db, import_id, summary_id, current_user.id
    )


@summaries_router.post(
    "/", response_model=MfAaSummaryResponse, status_code=status.HTTP_201_CREATED
)
async def create_aa_summary(
    ...
    return await aa_import_service.create_summary(
        db, import_id, current_user.id, payload
    )
...
async def update_aa_summary(
    ...
    return await aa_import_service.update_summary(
        db, import_id, summary_id, current_user.id, payload
    )
...
async def get_aa_import_transaction(
    ...
    return await aa_import_service.get_aa_transaction(
        db, import_id, txn_id, current_user.id
    )


@aa_txns_router.post(
    "/", response_model=MfAaTransactionResponse, status_code=status.HTTP_201_CREATED
)
async def create_aa_import_transaction(
    ...
    return await aa_import_service.create_aa_transaction(
        db, import_id, current_user.id, payload
    )
...
async def delete_aa_import_transaction(
    ...
    await aa_import_service.delete_aa_transaction(
        db, import_id, txn_id, current_user.id
    )
