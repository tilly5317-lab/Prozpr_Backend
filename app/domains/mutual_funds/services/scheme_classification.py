...
SUBCAT_TO_MAPPING: dict[str, tuple[str, str]] = {
    ...
    "Aggressive Hybrid Fund (Equity 65-80%, Debt 20-35%)":   ("Equity", "medium_beta_equities"),
...
_US_FOF_PATTERNS: tuple[str, ...] = (
    r"\bus\b", r"\bnasdaq\b", r"\bs&p\s*500\b", r"\bamerica\b",
)

_CHINA_FOF_PATTERNS: tuple[str, ...] = (
    r"\bchina\b", r"\bhang\s*seng\b",
)
...
def resolve_asset_bucket(
    ...
    if t not in _UNKNOWN_TYPE_TOKENS:
        ...
        if any(
            k in t
            for k in (
                "HYBRID", "BALANCED", "ARBITRAGE", "FOF", "SOLUTION",
                "GOLD", "COMMODITY", "MULTI ASSET",
            )
        ):
            return ASSET_CLASS_OTHERS
