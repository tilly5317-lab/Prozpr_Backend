...
- **The `mutual_fund_query` chat path forks before the agent.** `extract` sets `is_screen` for a generic "best performing funds" ask; that branch calls `screen_top_funds` and answers via the shared formatter with `action_mode="screen"` — the agent's `narrate` pass never runs. Only the named-fund path reaches the agent (`services/mutual_fund_query_service.py`).
