...
- **Never read `MfTransaction.amount` directly — go through `txn_value.trade_value(units, nav, amount)`.** Rows imported before the CAS Parser API can carry a mis-parsed amount (a "*** Stamp Duty ***" annotation line shifts the local parser's columns, so a Rs 99,995 purchase is filed as Rs 10); the row's own NAV contradicts it, so the value is taken from units x NAV whenever the two disagree by more than 2x. Observed spread on good rows is 0.9959–1.0041, so the band never fires on a real trade. Every reader — snapshot, holding detail, net-worth series, XIRR, TWR, benchmark clone, ingest-time derivation — uses it, or two pages price the same holding differently (`services/txn_value.py`).
