"""SQLAlchemy ORM model — `assumption.py`.

User-scoped cashflow assumptions (inflation, ROI, mortgage defaults). Mirrors
`Assumptions` in `AI_Agents/src/cashflow_statement/models.py`. One row per user.
"""
...
from sqlalchemy import CheckConstraint, DateTime, ForeignKey, Numeric, SmallInteger, func
...
class CashflowInputAssumption(Base):
    __tablename__ = "cashflow_input_assumptions"
    __table_args__ = (
        CheckConstraint("near_term_horizon_years >= 0", name="ck_cf_assump_near_term_nonneg"),
        CheckConstraint("medium_term_horizon_years >= 0", name="ck_cf_assump_mid_term_nonneg"),
        CheckConstraint("default_mortgage_tenure_years > 0", name="ck_cf_assump_mortgage_tenure_pos"),
        CheckConstraint(
    ...
    inflation_child_abroad_education: Mapped[float] = mapped_column(Numeric(7, 6), nullable=False)
    inflation_child_local_education: Mapped[float] = mapped_column(Numeric(7, 6), nullable=False)
    ...
    default_mortgage_interest_annual: Mapped[float] = mapped_column(Numeric(7, 6), nullable=False)
    default_property_downpayment_pct: Mapped[float] = mapped_column(Numeric(7, 6), nullable=False)
    ...
    user: Mapped["User"] = relationship(back_populates="cashflow_assumption")
