...
- `pii.py` — personal-data primitives, two jobs kept deliberately apart: `mask_*` renders ONE known field as a partial a human still recognises (`*****3210`); `redact_text`/`redact_obj` sweep an ARBITRARY blob we did not compose (third-party error bodies, exception messages, log records). Redaction is a backstop, never a licence to hand PII to a sink.
- `retention.py` — the retention policy and the job that enforces it. Prozpr is **not** SEBI-registered, so no statutory floor forces a period: every one is a product decision needing legal sign-off, not a legal minimum. Before this, nothing in the system was ever deleted.
- `encrypted_types.py` — SQLAlchemy column types encrypting at rest, aimed at third-party response blobs: never queried by content, so there is no index to preserve and no `WHERE` to rewrite. Keys come from `ENCRYPTION_KEY` via `get_fernet()`. **NOT for indexed or unique columns** — Fernet is non-deterministic, so equal inputs give different ciphertext.
...
- **The CAS scope is set in `get_current_user` (and re-set in `get_effective_user`).** That one indexed lookup per request is what lets ~56 query sites stay unedited. Code running OUTSIDE a request — schedulers, `BackgroundTasks` callbacks — carries no scope and will read across every statement a user has uploaded unless it wraps itself in `cas_scope_for_user` / resolves `effective_scope` first (`portfolio/services/networth/builder.py`, `identity/services/onboarding_generation_service.py` both do).
