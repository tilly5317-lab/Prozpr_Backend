...
must match the ``_REBAL_OVERRIDE_KEY_TO_USER_ATTR`` dict in ``chat.py``.

Mirrors the AA chat drift guard. If someone adds an override key to the
dict without updating the prompt's "Allowed override keys" block (or
...
from app.services.ai_bridge.rebalancing.chat import (
    _DETECT_REBAL_SYSTEM,
    _REBAL_OVERRIDE_KEY_TO_USER_ATTR,
)


def test_rebal_override_keys_in_prompt_match_code() -> None:
    """Keys listed under counterfactual_explore's 'Allowed override keys'
    block must match the keys in ``_REBAL_OVERRIDE_KEY_TO_USER_ATTR``.
    """
    ...
    code_keys = set(_REBAL_OVERRIDE_KEY_TO_USER_ATTR.keys())
    assert prompt_keys == code_keys, (
        f"Override key drift between _DETECT_REBAL_SYSTEM prompt and "
        f"_REBAL_OVERRIDE_KEY_TO_USER_ATTR dict in chat.py:\n"
